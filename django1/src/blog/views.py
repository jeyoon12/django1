...
class Detail(DetailView):
    template_name = 'blog/detail.html' # 이 변수 이름은 고정된 이름임
    model = Post
...
class Posting(LoginRequiredMixin, FormView):
    ...
    # 연동할 폼클래스 저장
    form_class = PostForm # 자동 완성 더블 클릭해서 임포트도 자동으로 되게 하기
    context_object_name = 'f'
    
    # is_valid()함수가 True를 반환한 뒤의 처리를 form_valid() 함수를 오버라이딩해 작성
    def form_valid(self, form):
        # 매개변수 form: is_valid()함수를 통과한 PostForm 객체
        # PostForm 객체를 바탕으로 Post 객체 저장
        ...
        # 클라이언트가 보낸 첨부파일, 이미지파일을 바탕으로 PostFile, PostImage 객체 생성 및 저장
        ...
        # reverse 자동 완성 시 django.urls.base 해야함
        # 자동 완성 다시 뜨게 하고 싶으면 그 변수 뒤에 커서 놓고 ctrl + space 누르면 됨
    
    # blog - templates - blog 우클릭해 new - other에서 총 네개의 html 파일 만들기
    # (base,detail,index,posting) 그 다음 base.html로 이동
